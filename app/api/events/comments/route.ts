...
import { users, events, eventRsvps, eventComments } from '@/lib/db/schema';
  ...
  try {
    ...
    if (viewerPrivyId) {
      ...
      if (viewer) {
        ...
        const interested = saved.includes(event.slug);
        if (interested) {
          canPost = true;
        } else {
          const [rsvp] = await db.select({ id: eventRsvps.id }).from(eventRsvps).where(and(eq(eventRsvps.eventId, event.id), eq(eventRsvps.userId, viewer.id))).limit(1);
          canPost = !!rsvp;
        }
      }
    }

    return NextResponse.json({ comments: rows, canPost });
  } catch (error) {
