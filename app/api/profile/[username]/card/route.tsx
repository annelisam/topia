...
// Let the CDN cache the rendered card so shares, link-preview crawls and email
// opens don't re-run Satori + re-ship the PNG every time. Cards refresh within
// ~1h (serving stale-while-revalidating for a day), which is fine for a
// shareable image — and cuts the two biggest variable costs (CPU + origin transfer).
const CARD_CACHE = 'public, max-age=300, s-maxage=3600, stale-while-revalidate=86400';
...
export async function GET(req: Request, { params }: { params: Promise<{ username: string }> }) {
  ...
  if (format === 'story') {
    return new ImageResponse(
      ...
      { width: 1080, height: 1920, fonts, headers: { 'Cache-Control': CARD_CACHE } },
    );
  }
  ...
  if (format === 'og') {
    ...
    return new ImageResponse(
      ...
      { width: 1200, height: 630, fonts, headers: { 'Cache-Control': CARD_CACHE } },
    );
  }
  ...
  if (stamp) {
    ...
    return new ImageResponse(
      ...
      { width: 1080, height: 1350, fonts, headers: { 'Cache-Control': CARD_CACHE } },
    );
  }
  return new ImageResponse(
    (
