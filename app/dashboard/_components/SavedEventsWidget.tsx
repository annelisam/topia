        ...
        {events.slice(0, 6).map((ev) => {
          ...
          return (
            <Link
              key={ev.id}
              href={`/events/${ev.slug}`}
              className="flex items-center gap-3 px-4 py-2.5 hover:bg-bone/[0.03] transition no-underline"
            >
              <div className="shrink-0 w-10 text-center border border-bone/15 rounded-sm py-1">
                <div className="font-basement text-[14px] leading-none text-bone">{chip.day}</div>
                <div className="font-mono text-[8px] uppercase tracking-[2px] text-bone/40 mt-0.5">{chip.mon}</div>
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-mono text-[12px] uppercase font-bold text-bone truncate">{ev.eventName}</div>
                <div className="font-mono text-[10px] text-bone/30 truncate">{ev.city || (ev.dateIso ? '' : 'TBD')}</div>
              </div>
            </Link>
          );
        })}
