...
export default function WorldLayer({ config, influences = [], feelsLike = [], currentlyBuilding = '', currentlyInto = [] }: Props) {
  ...
  return (
    <div className="relative bg-[var(--page-bg)] overflow-y-auto h-full" style={{ scrollbarWidth: 'thin' }}>
      ...
      <div className="relative z-10">
        {fields.map((field, i) => {
          ...
          return (
            <div key={field.label} className="flex items-start border-b border-ink/[0.04]" style={{ minHeight: '48px' }}>
              ...
              <div className="flex-1 px-3 py-3 min-w-0">
                <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 block mb-1.5">{field.label}</span>
                {isEmpty ? (
                  <span className="font-mono text-[11px] text-ink/20 italic">Not yet declared</span>
                ) : field.type === 'tags' ? (
                  <div className="flex flex-wrap gap-1.5">
                    {(field.data as string[]).map((tag) => (
                      <span key={tag} className="font-mono text-[10px] text-ink/50 border border-ink/[0.08] rounded-full px-2 py-0.5 whitespace-nowrap">{tag}</span>
                    ))}
                  </div>
                ) : (
                  <span className="font-zirkon text-[12px] text-ink/50 leading-relaxed block">{field.data as string}</span>
                )}
              </div>
            </div>
          );
