...
export default function PulseLayer({ config, availability = 'open', lastDrop, lastEvent, lastCollab }: Props) {
  ...
  return (
    <div className="relative bg-[var(--page-bg)] flex flex-col overflow-y-auto h-full" style={{ scrollbarWidth: 'thin' }}>
      ...
      <div className="flex items-center border-b border-ink/[0.04] relative z-10" style={{ minHeight: '48px' }}>
        <div className="w-[28px] shrink-0 flex items-center justify-center"><span className="font-mono text-[9px] text-ink/15">00</span></div>
        ...
        <div className="flex-1 px-3 py-3 flex items-center justify-between">
          <div>
            <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 block">availability</span>
            <div className="flex items-center gap-2 mt-1">
              <div className={`w-2 h-2 rounded-full ${avail.dotClass}`} />
              <span className="font-mono text-[12px] text-ink/60 uppercase tracking-wider">{avail.label}</span>
            </div>
          </div>
          <div className="text-right">
            <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 block">status</span>
            <span className="font-mono text-[11px] text-ink/40 mt-0.5 block">Currently active</span>
          </div>
        </div>
      </div>

      {rows.map((row, i) => (
        <div key={row.label} className="flex items-center border-b border-ink/[0.04] relative z-10" style={{ minHeight: '48px' }}>
          ...
          <div className="flex-1 px-3 py-3 min-w-0">
            <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 block">{row.label}</span>
            <span className="font-mono text-[12px] text-ink/50 mt-0.5 block truncate">{row.value}</span>
          </div>
        </div>
      ))}
      ...
      <div className="px-4 py-3 flex items-center gap-3 relative z-10">
        ...
        <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/15">live feed</span>
      </div>
    </div>
  );
}
