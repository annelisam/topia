...
export default function ProofLayer({ config, path, stats }: Props) {
  ...
  return (
    <div className="bg-[var(--page-bg)] flex flex-col h-full overflow-y-auto">
      ...
      <div className="grid grid-cols-2 gap-[1px] p-4">
        {cards.map((stat) => (
          <div key={stat.label} className="bg-[var(--page-bg)] border border-ink/[0.06] rounded-sm p-4 flex flex-col items-center justify-center text-center">
            <span className="font-mono text-[24px] md:text-[32px] font-bold text-ink leading-none">{stat.value}</span>
            <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 mt-2">{stat.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="px-4 pb-3 flex items-center gap-3">
        ...
        <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/15">topia://proof</span>
      </div>
    </div>
  );
}
