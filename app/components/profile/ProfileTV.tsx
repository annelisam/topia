...
export default function ProfileTV({ config, handle }: Props) {
  return (
    <div className="bg-[var(--page-bg)] flex flex-col overflow-hidden h-full">
      ...
      <div className="relative flex-1 min-h-0">
        ...
        <div className="absolute top-4 right-4 z-[5]">
          <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/40 border border-ink/[0.08] px-2 py-1 rounded-sm bg-[var(--page-bg)]/60 backdrop-blur-sm">Coming Soon</span>
        </div>

        <div className="absolute bottom-0 left-0 right-0 z-[4] bg-gradient-to-t from-[#1a1a1a] via-[#1a1a1a]/90 to-transparent p-4">
          <span className="font-mono text-[9px] uppercase tracking-[2px] text-ink/25 block mb-3">episode guide</span>
          <div className="grid grid-cols-2 gap-2">
            {EPISODES.map((ep) => (
              <div key={ep.title} className="border border-ink/[0.06] rounded-sm p-2.5 bg-[var(--page-bg)]/40 backdrop-blur-sm hover:bg-ink/[0.03] transition-colors cursor-pointer">
                <div className="flex items-center justify-between mb-1">
                  <span className={`font-mono text-[8px] uppercase tracking-wider px-1.5 py-0.5 rounded-sm ${ep.status === 'LIVE' ? 'bg-lime/20 text-lime' : ep.status === 'NEW' ? 'text-blue border border-blue/30' : 'text-ink/25 border border-ink/[0.08]'}`}>{ep.status}</span>
                  <span className="font-mono text-[9px] text-ink/25">{ep.duration}</span>
                </div>
                <span className="font-mono text-[10px] text-ink/50 font-bold uppercase block truncate">{ep.title}</span>
              </div>
            ))}
          </div>
        </div>
