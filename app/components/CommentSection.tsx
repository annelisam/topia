...
interface Props {
  /** Endpoint that handles GET/POST/DELETE. e.g. '/api/tools/comments' */
  endpoint: string;
  /** Resource slug (tool slug or event slug). */
  slug: string;
  /** "tool" enables the 5-star rating control + average display.
   *  "event" enables the GIF picker. */
  kind: 'tool' | 'event';
  /** Friendly verb for the empty/locked state. */
  gateHint: string;
  /** Section header label, e.g. "Reviews" or "Comments". */
  title?: string;
}
    ...
    <div className="border-t border-bone/[0.06] pt-5 mt-5">
        ...
        <div className="border border-bone/10 hover:border-bone/20 focus-within:border-lime/40 rounded-md p-3 mb-4 transition-colors bg-bone/[0.02]">
              ...
              {[1, 2, 3, 4, 5].map((n) => (
                ...
                >
                  <StarIcon
                    ...
                    className={n <= (hoverRating || rating) ? 'text-lime' : 'text-bone/25'}
                  />
                </button>
              ))}
              {rating > 0 && (
          ...
          {gif && (
            <div className="relative inline-block mt-1.5">
              ...
              <button
                onClick={() => setGif(null)}
                className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-obsidian border border-bone/20 text-bone text-[12px] cursor-pointer leading-none flex items-center justify-center"
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map((c) => (
            <div key={c.id} className="flex items-start gap-3 border border-bone/[0.06] rounded-md p-3 bg-bone/[0.015]">
              {c.authorAvatarUrl ? (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img src={c.authorAvatarUrl} alt="" className="w-7 h-7 rounded-full object-cover shrink-0" />
              ) : (
                <div className="w-7 h-7 rounded-full bg-bone/10 flex items-center justify-center shrink-0">
